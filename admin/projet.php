...
<body>
            ...
            if (isset($resultat[0]) == true){ //Check si résultat est définie ou pas     
                ...
                for($i=0; $i<$count ; $i++){
                    for($n=0; $n<$count ; $n++){
                        $projet = $resultat[$i];
                        $projetN = $resultat[$n];
                        if($projet["prj_nom"] === $projetN["prj_nom"]){  //Vérifie si il y a doublon
                            array_splice($resultat, $i, 1); //Supprime le contenu à l'index $i
                            $count = $count-1; //Réduire le count si on a supprimé un element
                        }    //End if qui check      
                        $n++;
                    }//End for seconde boucle
                    ...
                    }//End for d'affichage du résultat
                    echo('</div>');
    ...
    if(isset($_REQUEST['projet'])){
           ...
           //on rempli les 7 premiers champs vides pour arriver au calendrier
                echo("<div class='tableMainProjet'><table><thead><tr>
           ...
           for ($i = 1; $i< $entreeLength; $i++){
            ...
            echo("
                <form action='projet.php' method='POST' class='cacheMenuProjet'>
                    <input type='search' id='chercheTache' name='chercheTache' placeholder='Nom de la tâche' style='width: 30rem; text-align: center;'>
                    <input type='hidden' value='".$prjID."' name='projet'>
                    <input type='hidden' value='".$tache_id."' name='tch'>
                    <button>Rechercher une tâche</button>
                </form>
                <form action='projet.php' method='POST' class='cacheMenuProjet'>
                    <input type='search' id='chercheCat' name='chercheCat' placeholder='Catégorie' style='width: 30rem; text-align: center;'> 
                    <input type='hidden' value='".$prjID."' name='projet'>
                    <input type='hidden' value='".$tache_id."' name='tch'>
            ...
            if (isset($_POST['chercheCat']) == true){
                ...
                try {
                    ...
                    if (isset($resultat[0]) == true){ //Check si résultat est définie ou pas
                        ...
                        for($i=0; $i<$count ; $i++){
                            $tache = $resultat[$i];
                            //CHANGEMENT ICI CHANGEMENT ICI CHANGEMENT ICI CHANGEMENT ICI CHANGEMENT ICI CHANGEMENT ICI 
                            echo('
                            <div class="tache">
                            ');
